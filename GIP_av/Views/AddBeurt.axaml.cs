...
public partial class AddBeurt : Window
{
	PRIJZEN[] prijzen;
	SUCCESS success;
	public AddBeurt()
	...
	private async void Grid_Loaded_1(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
	{
	...
	private async Task SendInfo()
	{
		...
		try//proberen aanvraag naar server te versturen
		{
			...
			var response = await client.PostAsync(Data.server_address + "/add-user-beurt", content);//POST request versturen
			Debug.WriteLine("fetching");
